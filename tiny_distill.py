...
class TinySimCse(LightningModule):
    def __init__(self, train_data: pd.DataFrame, valid_data: pd.DataFrame, teacher_path: str, student_path: str):
        ...
        self.valid_metrics = CustomMetrics()
    ...
    def forward(self, input_ids, attention_mask, token_type_ids):
        teacher_cls = self.teacher_model(input_ids, attention_mask, token_type_ids)
        student_cls = self.student_model(input_ids, attention_mask, token_type_ids)

        loss = F.mse_loss(student_cls, teacher_cls, reduction='mean')
    ...
    def validation_step(self, batch, batch_idx):
        ...
        similarities = F.cosine_similarity(student_cls.unsqueeze(0), student_cls.unsqueeze(1), dim=2)
        row = torch.arange(0, student_cls.shape[0], 3).unsqueeze(1)
        col = torch.cat((torch.arange(1, student_cls.shape[0], 3).unsqueeze(1),
                         torch.arange(2, student_cls.shape[0], 3).unsqueeze(1)), dim=1)
        ...
        self.print(f'epoch: {self.current_epoch}, global_step: {self.global_step}, valid_step_loss: {loss:.5f}, valid_step_acc: {acc:.5f}')
    ...
    def validation_epoch_end(self, outputs):
        ...
        self.print(f'epoch: {self.current_epoch}, global_step: {self.global_step}, valid_loss: {loss:.5f}, valid_acc: {acc:.5f}')
        logger.info(f'epoch: {self.current_epoch}, global_step: {self.global_step}, valid_loss: {loss:.5f}, valid_acc: {acc:.5f}')
...
if __name__ == '__main__':
    ...
    checkpoint_callback = ModelCheckpoint(save_path=f'./simcse_distill', mode='min')
    trainer = pl.Trainer(
    ...
    cse = TinySimCse(train_data=train_data, valid_data=valid_data, teacher_path='./simcse', student_path='simcse_pred_distill')
    trainer.fit(cse)